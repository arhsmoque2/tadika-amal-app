...
| **ERR-031** | **57 Real PHPStan Findings, Several Statutory/Functional Bugs (not just noise)** | `./vendor/bin/phpstan --no-progress --memory-limit=512M` (real run, per ERR-030) | **Checker-Automated (CI-run) + Agent-Manual (fix)** | Triaged all 57 errors from ERR-030's real run by reading each flagged call site against the actual model schema, not guessing from the message text alone. Found genuine bugs, not just type-inference noise: `WordTemplateExportService::generateIncidentReportDocx()` (the official JKM statutory incident report) read `$incident->incident_number`/`description`/`action_taken`/`jkm_reportable`/`reporter` — none of which exist on `IncidentLog` — so every generated JKM report silently had blank incident number, description, action-taken, and reporter fields; added `incidentNumber`/`jkmReportable` accessors and corrected the field names. `AiReportDiagnosticService` queried `health_screenings` by a nonexistent `screening_date` column and a nonexistent `has_hfmd_symptoms` column (real columns: `date`, and `symptoms` JSON array) — a genuine `QueryException` on every call, not a silent bug; also read `$eval->skill?->category` (real: `domain_category`) and `$eval->notes` (real: `remarks`) for the AI Teacher Co-Pilot payload. `PresentationDeckService` (`.pptx` generation) referenced `phpoffice/phppresentation` classes that were never added to `composer.json` — the code already guards every use behind `class_exists()` with a graceful text fallback, so this doesn't crash, but `.pptx` decks silently degrade to plain text; scoped a documented `phpstan.neon` ignore to that one file rather than adding an unverified new dependency from a sandbox that cannot confirm it installs cleanly (see Gap below). Also fixed: 5 `str_pad(int, ...)` type errors (cast to string), `AdminFactory` typed `@extends Factory<User>` instead of `Factory<Admin>`, `User`/`Admin` `$fillable` missing `email_verified_at`/`remember_token` despite both being real migrated columns, `LessonPlan` field-name typos (`kspk_strand`→`tunjang_kspk`, `learning_activities`→`activities`) in `PresentationDeckService`, one dead defensive ternary on a non-nullable column, and added Larastan generic `@return BelongsTo<X, $this>` PHPDoc to all 76 relation methods across all 21 models to resolve the ~25 "undefined property on base `Model`" errors that were type-inference gaps rather than real bugs. Two `match()` arms flagged "comparison always true" are legitimate defensive fallback code for data integrity (kept, with a scoped `@phpstan-ignore` and rationale) — removing them would only change behavior for corrupted/legacy data, which is exactly the case the `default` arm exists to handle. **Gap**: 5 "nullsafe unnecessary" findings were deliberately left unfixed — at least one (`DocumentTemplateService`'s `$student->cohort?->name`) points at a genuinely nullable relation (`students.cohort_id` is nullable), so blindly removing the `?->` per PHPStan's suggestion would risk a real null-pointer error; this needs a real phpstan run with file:line detail to triage safely, not a guess. None of this was verified with a local `phpstan` run (same sandbox network limitation as ERR-023) — verification is CI's next real run, per `AGENTS.md` Prime Directive #5. **Verified**: real CI run on PR #7 confirmed 57→13 after the first push (2 of the 13 were bugs in this fix's own first pass — a duplicated always-true-ternary in the new `incidentNumber` accessor, and a guessed `@phpstan-ignore` identifier that didn't match PHPStan's actual rule name); a follow-up commit fixed both and CI confirmed the final count is exactly the 5 deliberately-unfixed nullsafe findings, nothing else — `./vendor/bin/phpstan` is clean of everything this fix set out to address. |
| **ERR-032** | **GCP Cloud Run Migration Failure (`DATABASE_URL` Driver Config Mismatch)** | Real Cloud Run Migration Job (`gcloud run jobs execute tadika-migrate`) / `_qa/tadika-infra-quality-gate.mjs` Gate 10 | **Checker-Automated & Verified** | Deployment pipeline and GCP Secret Manager inject Neon PostgreSQL connection string as `DATABASE_URL`. Laravel's `config/database.php` pgsql driver only queried `env('DB_URL')` without fallback, defaulting connection to `127.0.0.1:5432` and failing migrations every deploy. Added `'url' => env('DB_URL', env('DATABASE_URL'))` and added automated test/gate assertions in `_qa/tadika-infra-quality-gate.mjs` and `tests/Feature/InfrastructureConfigTest.php`. |
| **ERR-033** | **Neon PgBouncer Transaction Abort on Rate Limiting (`SQLSTATE[25P02]` with `CACHE_STORE=database`)** | Livewire Update Exception / Live Authentication Testing on Cloud Run (`tadika-amal-app`) | **Agent-Manual & Cloud-Config Verified** | Livewire authentication rate limiting (`RateLimiter::hit()`) invoked atomic cache increments. With `CACHE_STORE=database`, Laravel's `DatabaseStore` ran `DB::transaction()` with `SELECT ... FOR UPDATE` pessimistic locks over Neon's PgBouncer transaction pooler (`-pooler.neon.tech`), causing transaction abort (`25P02`). Switched `CACHE_STORE=file` on Cloud Run and `.github/workflows/deploy.yml` while maintaining `SESSION_DRIVER=database` for stateless multi-instance sessions. |
...
### 19. Triaging ERR-030's 57 Findings (ERR-031)
- **Script / Command**: read each of the 57 error messages from the real CI run's job log against the actual model `$fillable`/migration schema — not a phpstan run (blocked by the same sandbox network limit as ERR-023), the real error text from ERR-030's real CI execution.
- **Symptom**: mechanically "fixing" 57 messages by pattern alone (e.g. always removing a flagged `?->`) risks introducing real bugs where PHPStan's inference is wrong (a genuinely nullable relation) or missing real bugs where it's right but for a non-obvious reason.
- **Why It's Dangerous**: several of these were not noise. `WordTemplateExportService::generateIncidentReportDocx()` (JKM statutory compliance report) and `AiReportDiagnosticService` (AI Teacher Co-Pilot payload, and a genuine `QueryException`-causing bad column name in a health-screening query) were reading fields that don't exist on the model — silent data-integrity bugs in compliance-critical and crash bugs in AI-pipeline code, both unrelated to the "just add PHPDoc" fix that resolved the bulk of the noise.
- **Fix**: see the ERR-031 matrix row for the full breakdown — real field-name corrections, two new model accessors, a factory generic-type fix, two `$fillable` completions, a dead-ternary removal, a documented `phpstan.neon` ignore for the one file using an intentionally-optional undeclared dependency, and Larastan relation generics added to all 21 models (76 methods) to resolve the bulk "undefined property on base `Model`" class of finding.
- **Verification**: `php -l` and `pint --test` pass on every changed file locally. The actual PHPStan error count after this fix is **not yet verified** — same sandbox limitation as ERR-023/030, verification is the next real CI run on this PR.

### 20. Cloud Run Migration Failure & Database URL Mismatch (ERR-032)
- **Script / Command**: `gcloud run jobs execute tadika-migrate` (in `.github/workflows/deploy.yml`) and `node _qa/tadika-infra-quality-gate.mjs`.
- **Symptom**: `ERROR: (gcloud.run.jobs.execute) The execution failed.` on `php artisan migrate --force` Cloud Run Job step; deployment pipeline failed repeatedly.
- **Why It's Dangerous**: Deployment pipeline could not apply schema changes or seeders to Neon Serverless PostgreSQL, leaving the production Cloud Run service unable to boot or serve user traffic.
- **Fix**: Updated `config/database.php` pgsql driver configuration to `'url' => env('DB_URL', env('DATABASE_URL'))`. Added automated static assertion in `_qa/tadika-infra-quality-gate.mjs` and PHPUnit feature assertion in `tests/Feature/InfrastructureConfigTest.php` to prevent future configuration regression.
- **Verification**: GitHub Actions workflow runs `32653536478` and `32654718301` completed with `100% Success`. Migration job executed cleanly against Neon Postgres; live `/up` health probe returns HTTP `200 OK`.

### 21. Neon PgBouncer Transaction Abort on Cache Rate Limiting (ERR-033)
- **Script / Command**: Livewire authentication form submission (`POST /livewire/update` at `/admin/login` and `/app/login`) on Cloud Run.
- **Symptom**: `SQLSTATE[25P02]: In failed sql transaction: 7 ERROR: current transaction is aborted, commands ignored until end of transaction block (Connection: pgsql, Host: ep-ancient-sky-azbug5v0-pooler... SQL: update "cache" set "value" = i:1; where "key" = tadika_amal_cache_livewire-rate-limiter:...)`.
- **Why It's Dangerous**: Teachers and administrators could not authenticate through Livewire login forms on Cloud Run because the login rate-limiter immediately crashed with an internal 500 error.
- **Root Cause**: Neon's pooled connection strings (`-pooler`) use PgBouncer in transaction pooling mode. Laravel's `DatabaseStore` uses `DB::transaction()` with `SELECT ... FOR UPDATE` pessimistic locks during atomic `increment()` operations. In PgBouncer transaction pooling, statement-level lock conflicts cause PostgreSQL to abort the transaction block.
- **Fix**: Switched `CACHE_STORE=file` in Cloud Run environment variables and `.github/workflows/deploy.yml`. File-based cache executes directly in local ephemeral storage (`storage/framework/cache/data`) with microsecond response times and zero database locking, while `SESSION_DRIVER=database` preserves session state across autoscaling instances.
- **Verification**: Updated Cloud Run revision `tadika-amal-app-00003-qd7`, confirmed rate-limiting cache operations execute without database transaction errors, and pushed permanent configuration to `deploy.yml`.
